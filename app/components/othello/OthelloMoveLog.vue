...
import { BLACK } from '~/game/othello/engine'
import type { LogEntry } from '~/composables/useOthelloGame'
...
<template>
  <div class="flex flex-col">
    ...
    <ol v-else ref="list" class="min-h-0 flex-1 space-y-0.5 overflow-y-auto pr-1">
      ...
      >
        <span class="w-5 shrink-0 text-right font-mono tabular-nums text-ink-subtle">{{ entry.turn }}</span>

        <span
          class="size-3 shrink-0 rounded-full"
          :class="entry.player === BLACK ? 'bg-gradient-to-br from-[#413354] to-[#1c1524]' : 'border border-line-strong bg-white'"
        />

        <span class="w-7 font-mono font-medium text-ink">{{ entry.notation }}</span>

        <span class="truncate text-[11px] text-ink-subtle">
          {{ entry.pass ? 'ผ่านตา' : `พลิก ${entry.flips} หมาก` }}
        </span>
      </li>
    </ol>
  </div>
