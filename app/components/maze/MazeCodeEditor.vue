...
import { AGENT_TEMPLATES } from '~/game/maze/templates'
...
<template>
  <UiModal
    ...
    wide
    title="โค้ดหาทางออก"
    description="เขียน class Agent extends MazeAgent แล้ว override solve(state) หรือ step(state)"
  >
    <div class="space-y-4">
    ...
    <template #footer>
      <div class="flex items-center justify-between gap-3">
        <p class="text-xs text-ink-muted">โค้ดรันใน Web Worker แยกจากหน้าเว็บ และมีเวลาคิด 5 วินาที</p>

        <div class="flex gap-2">
          <UiButton variant="outline" size="sm" :disabled="running" @click="runTest">
            {{ running ? 'กำลังทดสอบ…' : 'ทดสอบบนแผนที่นี้' }}
          </UiButton>
