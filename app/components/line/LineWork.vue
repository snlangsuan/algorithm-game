...
import { WORK_LABEL } from '~/game/line/blocks/pack'
...
const perSecond = computed(() => (props.seconds > 0 ? Math.round(props.calls / props.seconds) : 0))
</script>
  ...
  <div v-if="work.length > 0" class="mt-2 rounded-xl bg-surface-muted p-3">
    <p class="flex items-center gap-1.5 text-xs font-semibold text-ink">
      ...
      <UiInfo label="ตัวเลขพวกนี้อ่านยังไง" align="left">
        บอทถูกถาม 30 ครั้งต่อวินาที ทุกครั้งต้องตอบกำลังมอเตอร์หนึ่งคำสั่ง ·
        บล็อก "ตำแหน่งเส้น" อ่านจากสภาพที่เกมส่งมาให้ตรง ๆ จึงไม่ถูกนับเป็นการอ่านเซนเซอร์
      </UiInfo>
    </p>
