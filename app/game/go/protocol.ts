...
export type WorkerRequest =
  ...
  /** lead = แต้มนำของฝ่ายดำหลังรวมโคมิ — โกะจบด้วยการนับแต้ม ไม่ใช่แค่รู้ว่าใครชนะ */
  | { type: 'end'; board: Board; winner: Player | null; lead: number }
...
/** ตาที่ agent ตอบกลับมา — ลงช่องไหน หรือผ่านตา */
export type MoveReply = { row: number; col: number } | 'pass'

export type WorkerResponse =
  | { type: 'log'; lines: LogLine[] }
  | { type: 'ready'; name: string; traced: boolean }
  | { type: 'memory'; data: AgentMemory }
  | { type: 'move'; id: number; move: MoveReply }
